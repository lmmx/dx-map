...
impl MapLibreManager {
    ...
    pub fn add_map_controls(&mut self) -> Result<(), JsValue> {
        with_context("MapLibreManager::add_map_controls", LogCategory::Map, |logger| {
    ...
    pub fn setup_map_data(&mut self, simulation_enabled: bool) -> Result<(), JsValue> {
        with_context("MapLibreManager::setup_map_data", LogCategory::Map, |logger| {
            if let Some(map) = &self.map {
                ...
                let load_handler = Closure::wrap(Box::new(move || {
                    ...
                    let window = match window() {
                        Some(w) => w,
                        None => {
                            log::error_with_category(LogCategory::Map, "Window not available in load handler");
                            return;
                        }
                    };
                    ...
                    let map_instance =
                        match js_sys::Reflect::get(&window, &JsValue::from_str("mapInstance")) {
                            Ok(m) => {
                                if m.is_undefined() {
                                    log::error_with_category(LogCategory::Map, "mapInstance is undefined");
                                    return;
                                }
                                m
                            }
                    ...
                    if let Err(err) = result {
                        ...
                        log::info_with_category(LogCategory::Map, "Map layers added successfully");
                    }
                }) as Box<dyn FnMut()>);
    ...
    pub fn update_layer_visibility(&self, layers: &crate::app::TflLayers) -> Result<(), JsValue> {
        with_context("MapLibreManager::update_layer_visibility", LogCategory::Map, |logger| {
            if let Some(map) = &self.map {
    ...
    pub fn debug_check_maplibregl() -> Result<(), JsValue> {
        with_context("MapLibreManager::debug_check_maplibregl", LogCategory::Map, |logger| {
