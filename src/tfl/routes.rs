        ...
        for line in lines {
            ...
            if fields.len() >= 5 {
                ...
                if let (Some(start_coords), Some(end_coords)) = (start_coords, end_coords) {
                    ...
                    console::warn_1(
                        &format!("Skipping tube route due to invalid coordinates: {}", line).into(),
                    );
                }
            } else {
                console::warn_1(
                    &format!("Skipping tube route due to invalid format: {}", line).into(),
                );
            }
        }
    }
            ...
            if fields.len() >= 5 {
                if let Ok(route_number) = fields[0].trim().parse::<i32>() {
                    ...
                    if let (Some(start_coords), Some(end_coords)) = (start_coords, end_coords) {
                        ...
                        console::warn_1(
                            &format!("Skipping bus route due to invalid coordinates: {}", line)
                                .into(),
                        );
                    }
                } else {
                    console::warn_1(
                        &format!("Skipping bus route due to invalid route number: {}", line).into(),
                    );
                }
            } else {
                console::warn_1(
                    &format!("Skipping bus route due to invalid format: {}", line).into(),
                );
            }
        }
    }
