use super::model::{Platform, Station, PlatformsResponse, StationsResponse};
...
pub async fn load_stations() -> Result<Vec<Station>, String> {
    ...
    let response: Response = response_value.dyn_into().map_err(|_| "Failed to convert response")?;
    ...
    let text_promise = response.text().map_err(|e| format!("Failed to get response text: {:?}", e))?;
...
pub fn group_platforms_by_station(platforms: Vec<Platform>) -> std::collections::HashMap<String, Vec<Platform>> {
